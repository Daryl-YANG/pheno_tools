PRO PhenoFit_Beck_Derv_V1
  ...
  ;********************************* SET OUPUT DIRECTORY ****************************************;
  out_dir = '\\modex.bnl.gov\data2\dyang\projects\ngee_arctic\seward\analysis\phenology\kougarok\planetscope\2022\phenoR\pheno_idl_20230211'
  FILE_MKDIR, out_dir
...
Function phenofit, pixel_ndvi, date_stamp, expBEG, expEND, doy_start, doy_end, intv, pheno_par=pheno_par, fitted_ndvi=fitted_ndvi
  ...
  pixel_ndvi[subsnow] = 0.02 ; 0.05
  ; replace extreme low values using baseline values
  pixel_ndvi[where(pixel_ndvi lt 0.02)] = 0.02 ;0.05
  ; detect outliers leveraging sg filter
  sg_smooth = sgfilter(pixel_ndvi)
  ndvi_diff = pixel_ndvi - sg_smooth
  subout = where(abs(ndvi_diff) gt 0.2)
  pixel_ndvi[subout] = 0
  ; fill up outlies
...
Function extend, time_stamp, ndvi, ext_doy, expBEG, expEND, ext_ndvi = ext_ndvi
  ...
  end_extend_ndvi = replicate(ndvi_ref[0], n_end_dates) + randomu(seed, n_end_dates)*0.02 ;min(ndvi)
