...
Function phenofit, pixel_ndvi, date_stamp, expBEG, expEND, doy_start, doy_end, intv, pheno_par=pheno_par, fitted_ndvi=fitted_ndvi
  ...
  if num gt 15 then begin
    ...
    fit_par_combn = []
    for itr = 0, 19 do begin
      parms = [0.01, 1.0, 0.05, 0.02, 140, 260]
      yerr = randomn(seed, n_elements(ext_doy))/10
      ...
      if n_elements(result) eq 6 then begin
        ...
        pars_itr = [mn, mx, rsp, rau, sos, eos]
        fit_par_combn = [[fit_par_combn], [pars_itr]]
      endif
    endfor
    
;    result = []
;    for colm=0, 5 do begin
;      colm_data = fit_par_combn[colm, *]
;      resistant_mean, colm_data, 1, meanv
;      result = [result, meanv]
;    endfor
    
    resistant_mean, fit_par_combn, 1, meanv, dimension = 2
    result = meanv
